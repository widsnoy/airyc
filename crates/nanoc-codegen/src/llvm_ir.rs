    ...
    fn compile_binary_expr(&mut self, expr: BinaryExpr) -> BasicValueEnum<'ctx> {
            ...
            && matches!(op_token.kind(), SyntaxKind::AMPAMP | SyntaxKind::PIPEPIPE)
        {
            ...
            let eq_zero = self
                ...
                .unwrap();
            if op_token.kind() == SyntaxKind::AMPAMP {
                let _ = self
                    .builder
            ...
            let merge = self
                ...
                .unwrap();
            let left_val = self.builder.build_not(eq_zero, "land.not").unwrap();
            let right_val = as_bool(self.builder, rhs.into());
            merge.add_incoming(&[(&left_val, lhs_bb), (&right_val, rhs_bb)]);
